...
if (Object.keys(index).length === 0) {
    changed = readdirSync(WIKI_DIR)
        .filter((f) => f.endsWith(".md"))
        .map((f) => join(WIKI_DIR, f));
} else {
    const c = execSync(`git diff --name-only --diff-filter=ACMR HEAD~1 HEAD -- ${WIKI_DIR}/`)
        .toString()
        .trim();
    changed = c ? c.split("\n").filter((f) => f.endsWith(".md")) : [];
    const d = execSync(`git diff --name-only --diff-filter=D HEAD~1 HEAD -- ${WIKI_DIR}/`)
        .toString()
        .trim();
    deleted = d ? d.split("\n").filter((f) => f.endsWith(".md")) : [];
}
